...
/*   Updated: 2024/05/07 18:41:47 by gfinet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (*start && check_type) // (*start)->type != WORD_TK) // READ_TK, WRITE_TK, APPEN_TK
	{
...
void empty_end(t_arg_lst **start, t_arg_lst **prev)
{
	...
	while ((*start) && !in_int_array((*start)->type, (int[]){READ_TK, WRITE_TK, APPEN_TK}, 3))
	{
		*prev = *start;
		*start = (*start)->next;
	}
	while (*start && in_int_array((*start)->type, (int[]){READ_TK, WRITE_TK, APPEN_TK}, 3))
		jump_free_arg(start, *prev, in_int_array((*start)->type, (int[]){READ_TK, WRITE_TK, APPEN_TK}, 3));
}
